...
4. **For every word that appears in UI**, check `righter` skill.
5. Then read `vois-design-system` for implementation (tokens, spacing, components).
...
# TEMPLATE 2: Table/List with Details
...
# TEMPLATE 4: Dialog / Action Sheet
...
# TEMPLATE 5: Detail Page (Read-Only / View State)
...
2. For every word, check righter skill
3. Get the exact copy from righter
4. Then implement with vois-design-system rules (tokens, spacing, components)
```
